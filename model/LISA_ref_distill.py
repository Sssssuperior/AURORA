...
class LisaModel(LisaMetaModel, Videollama2Qwen2Model):
    def __init__(
        ...
        **kwargs,
        ...
        # from VideoLLaMA2av import videollama2
        # from transformers import AutoModelForCausalLM
        # self.av_model = AutoModelForCausalLM.from_pretrained("/home/yves/data/liunian/lzy/videollama/VideoLLaMA2.1-7B-AV/")


class LISAForCausalLM_ref(Videollama2Qwen2ForCausalLM):
    def __init__(
        self,
        config,
        **kwargs,
    ...
    def model_forward(
        ...
        **kwargs,
        ...
        labels1 = output['labels'].cuda()
        # class_name = output[0]['class_name']

        keywords = [tokenizer.eos_token]
        from VideoLLaMA2av.videollama2.mm_utils import KeywordsStoppingCriteria
        stopping_criteria = KeywordsStoppingCriteria(keywords, tokenizer, output['input_ids'].cuda())
        ...
        attention_masks = attention_masks[mask].unsqueeze(0)
        # (4.6) 这里不用generate，直接生成it is [seg]的形式
        with torch.inference_mode():
        # (4.13) 这里使用generate，之后通过generate输出的结果得到hidden state
            fallback_text = "It is {} [SEG].".format(class_name)
            ...
            labels = torch.cat((labels1[mask].unsqueeze(0), output_ids), dim=-1)
            ...
            output = super(LISAForCausalLM_ref, self).forward(
                    input_ids=input_ids,
        ...
        with torch.inference_mode():
            ...
            # # 因为是二值分割，所以每个只需要一个token分割即可
            # offset = batch_size + 1
            # seg_token_offset = seg_token_offset[offset]
            seg_token_offset = seg_token_counts.cumsum(-1)

            # pred_embeddings_ = []
            # for i in range(len(seg_token_offset)):
            #     start_i, end_i = seg_token_offset[i], seg_token_offset[i + 1]
            #     pred_embeddings_.append(pred_embeddings[start_i:end_i])
            # pred_embeddings = pred_embeddings_
            # pred_embeddings = torch.split(pred_embeddings, seg_token_offset.tolist(), dim=0)
