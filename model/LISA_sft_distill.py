        ...
        **kwargs,
        ...
        if inference:
            ...
            # (4.13) 这里使用generate，之后通过generate输出的结果得到hidden state
                ...
                with open("output.json", "a", encoding="utf-8") as f:
                    json.dump({"classes": class_name, "outputs": outputs}, f, ensure_ascii=False)
                    f.write("\n")  # 每条结果换一行
                  # 每条结果换一行
            ...
            output_hidden_states = output.hidden_states #这里应该返回的是每一层的hiddesn state
        ...
        if inference:
            for batch_idx in range(len(pred_masks)):
                ...
                pred_mask1 = torch.sigmoid(pred_mask)  # 应用 Sigmoid 处理
                pred_mask1 = (pred_mask1 * 255).byte()

                # 依次保存每张 mask
                for i in range(pred_mask1.shape[0]):
                    from PIL import Image
                    mask_image = Image.fromarray(pred_mask1[i].cpu().numpy())  # 转换为 PIL 图像
