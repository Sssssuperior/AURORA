...
def model_init(model_path=None, **kwargs):
    ...
    tokenizer = load_pretrained_model(model_path, None, model_name, **kwargs)
...
def mm_infer(image_or_video, instruct, model, tokenizer, modal='video', **kwargs):
    ...
    if modal == 'text':
        ...
        if isinstance(image_or_video, dict):
            tensor = {k: v.bfloat16().cuda() for k, v in image_or_video.items()}
        else:
            tensor = image_or_video.bfloat16().cuda() 
        tensor = [(tensor, modal)]
